namespace Tempname
{
    ...
    public static partial class InputUtility
    {
        ...
        public static Vector2 ProcessLookRotationDelta(FrameRecord[] frameRecords, Vector2 lookRotationDelta,
            float lookRotationSensitivity, float lookResponsivity)
        {
        ...
        private static void CleanLookRotationDeltaHistory(FrameRecord[] frameRecords, Vector2 lookRotationDelta)
        {
            ...
            if (lookRotationDelta.x < 0.0f)
            {
                ...
                {
                    if (frameRecords[i].LookRotationDelta.x > 0.0f) { frameRecords[i].LookRotationDelta.x = 0.0f; }
                }
            }
            ...
            if (lookRotationDelta.y < 0.0f)
            {
                for (int i = 0; i < count; ++i)
                {
                    if (frameRecords[i].LookRotationDelta.y > 0.0f) { frameRecords[i].LookRotationDelta.y = 0.0f; }
                }
            }
